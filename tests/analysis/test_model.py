...
import pandas as pd
import pytest
from assignment_4.monte_carlo.model import (
    _generate_cov_matrix,
    _generate_independent_and_dependent_variables,
    _generate_measurement_error,
    do_monte_carlo,
)
...
def test_generate_independent_and_dependent_variables_correct_shape(inputs):
    rng = np.random.default_rng(inputs["seed"])
    mean, cov_type, n_obs, y_sd, true_params = (
        inputs["mean"],
        inputs["cov_type"],
        inputs["n_obs"],
        inputs["y_sd"],
...
def test_input_negative_std_deviation_of_meas_error(inputs):
    ...
    assert (
        ...
        == "Standard deviation of measurement error cannot be negative."
    )
...
def test_input_negative_y_std(inputs):
    ...
    assert "Standard deviation of dependent variable y cannot be negative." in str(
        excinfo.value,
    )
...
def test_input_string_parameter_vector(inputs):
    ...
    assert str(excinfo.value) == "Parameter cannot be a string."
